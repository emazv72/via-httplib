...
+ The `asio` C++ library, see [asio](http://think-async.com/Asio/).
...
THis version of `via-http` lib depends on the `asio` libraries.
If `asio` is not installed on your machine then download the latest package from
[asio](http://think-async.com/Asio/). Note: `asio` is a header-only library.
